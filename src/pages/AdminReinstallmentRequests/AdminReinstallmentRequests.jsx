...
import './AdminReinstallmentRequests.css'

const AdminReinstallmentRequests = () => {
  ...
  const loadRequests = async () => {
    try {
      ...
      setErrorMessage('Unable to load reinstatement requests.')
    } finally {
      setLoading(false)
    }
  }
    ...
    try {
      ...
      const res =
        reviewAction === 'approve'
          ? await memberService.approveReinstallment(
              selectedRequest.id,
              currentUser?.email,
              reviewComment
            )
          : await memberService.rejectReinstallment(
              selectedRequest.id,
              currentUser?.email,
              reviewComment
            )
      ...
      if (res?.success) {
        setSuccessMessage(
          reviewAction === 'approve'
            ...
            : '❌ Reinstatement request rejected successfully.'
        )
  ...
  const getUserInfo = request => request.user_email || request.user_name || `User #${request.user_id}`
  ...
  return (
    <div className="admin-reinstallment-page">
      <h1>Reinstatement Requests</h1>
      <p className="page-subtitle">
        Review and manage suspension reinstatement requests for{' '}
      ...
      {loading ? (
        <p>Loading reinstatement requests...</p>
      ) : filteredRequests.length === 0 ? (
        <div className="empty-state">
          <p>
            {filterStatus === 'All'
              ? 'No reinstatement requests found.'
...
export default AdminReinstallmentRequests
