...
const Invitations = () => {
  ...
  const confirmSuspend = async () => {
    ...
    const memberId = pendingMember.id || pendingMember.member_id || pendingMember.user_id
    ...
    try {
      ...
      alert(`${pendingMember.name || pendingMember.email} has been suspended.`)
      await loadMembers()
    } catch (error) {
      console.error('Suspend member error:', error)
      alert(error?.response?.data?.detail || 'Unable to suspend member')
    } finally {
  ...
  const confirmReactivate = async () => {
    ...
    try {
      ...
      alert(`${pendingMember.name || pendingMember.email} has been reactivated.`)
      await loadMembers()
    } catch (error) {
      console.error('Reactivate member error:', error)
      alert(error?.response?.data?.detail || 'Unable to reactivate member')
    } finally {
  ...
  return (
    <>
      ...
      {showSuspendModal && (
        <ConfirmModal
          title={`Suspend ${pendingMember?.name || pendingMember?.email}`}
      ...
      {showReactivateModal && (
        <ConfirmModal
          title={`Reactivate ${pendingMember?.name || pendingMember?.email}`}
